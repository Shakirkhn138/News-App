  ...
  factory Article.fromJson(Map<String, dynamic> json) {
    return Article(
        ...
        source: json['source'] != null
            ? json['source']['name'] ?? 'Source not available'
            : 'Source not available',
        publishedAt: json['publishedAt'] ?? 'No published data available');
  }
}

class EspnNews extends StatefulWidget {
  const EspnNews({super.key});

  @override
  State<EspnNews> createState() => _EspnNewsState();
}

class _EspnNewsState extends State<EspnNews> {
  
  Future<List<Article>> espnNews () async {
    
    final response = await http.get(Uri.parse('https://newsapi.org/v2/top-headlines?sources=espn&apiKey=577e690eecc7494487f8d1ca921485c8'));
    ...
    if ( response.statusCode == 200 ){
      ...
      throw Exception('Failed to fetch espn news');
    }
